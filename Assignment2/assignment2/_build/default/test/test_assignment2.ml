...
let%test "type_test2" = (type_of (Add (ConstV [1.;2.], ConstV [3.;4.])) = Vector 2)
...
let%test "type_test11" = 
  (try
    let _ = type_of (Add (T, ConstS 1.)) in false
  with
    | Wrong e -> e = Add (T, ConstS 1.)
    | _ -> false) 
let%test "type_test12" =
  (try
    let _ = type_of (Add (ConstS 2., ConstV [1.;2.])) in false
  with
    | Wrong e -> e = Add (ConstS 2., ConstV [1.;2.])
    | _ -> false)
let%test "type_test13" =
  (try
    let _ = type_of (Add (ConstV [1.;2.], T)) in false
  with
...
let%test "type_test29" = (type_of (ScalProd ((Inv (ConstS 1.)), ConstS 1.)) = Scalar)
let%test "type_test30" = (type_of (ScalProd ((Inv (ConstV [1.;2.])), ConstS 2.)) = Vector 2)
let%test "type_test31" = (type_of (ScalProd (ConstV [1.;2.], ConstS 1.)) = Vector 2)
let%test "type_test32" = 
  try
    let _ = type_of (ScalProd (T, ConstS 1.)) in false
  with
    | Wrong e -> e = ScalProd (T, ConstS 1.)
    | _ -> false
let%test "type_test33" =
  try
    let _ = type_of (ScalProd (T, ConstV [1.;2.])) in false
  with
    | Wrong e -> e = ScalProd (T, ConstV [1.;2.])
    | _ -> false
let%test "type_test34" =
  try
    let _ = type_of (ScalProd (ConstV [1.; 3.], ConstV [1.;2.])) in false
  with
    | Wrong e -> e = ScalProd (ConstV[1.; 3.], ConstV [1.;2.])
    | _ -> false
let%test "type_test35" = (type_of (DotProd (ConstV [1.;2.], ConstV [3.;4.])) = Scalar)
let%test "type_test36" = (type_of (DotProd (ConstV [1.;2.;3.;4.], ConstV [3.;4.;5.;6.])) = Scalar)
let%test "type_test37" = (type_of (DotProd (ConstV [1.;2.;3.;4.;5.], ConstV [3.;4.;5.;6.;7.])) = Scalar)
let%test "type_test38" = 
  try
    let _ = type_of (DotProd (ConstV [1.;2.], ConstV [3.;4.;5.])) in false
  with
    | Wrong e -> e = DotProd (ConstV [1.;2.], ConstV [3.;4.;5.])
    | _ -> false
let%test "type_test39" = 
  try
    let _ = type_of (DotProd (ConstV [1.;2.], ConstS 1.)) in false
  with
    | Wrong e -> e = DotProd (ConstV [1.;2.], ConstS 1.)
    | _ -> false
let%test "type_test40" =
  try
    let _ = type_of (DotProd (T, ConstV [1.;2.])) in false
  with
...
let%test "type_test46" =
  try
    let _ = type_of (Mag (F)) in false
  with
    | Wrong e -> e = Mag (F)
    | _ -> false
let%test "type_test47" = (type_of (Angle (ConstV [1.;2.], ConstV [3.;4.])) = Scalar)
let%test "type_test48" = (type_of (Angle (ConstV [1.;2.;3.;4.], ConstV [3.;4.;5.;6.])) = Scalar)
let%test "type_test49" = (type_of (Angle (ConstV [1.;2.;3.;4.;5.], ConstV [3.;4.;5.;6.;7.])) = Scalar)
let%test "type_test50" = 
  try
    let _ = type_of (Angle (ConstV [1.;2.], ConstV [3.;4.;5.])) in false
  with
    | Wrong e -> e = Angle (ConstV [1.;2.], ConstV [3.;4.;5.])
    | _ -> false
let%test "type_test51" =
  try
    let _ = type_of (Angle (ConstV [1.;2.], ConstS 1.)) in false
  with
    | Wrong e -> e = Angle (ConstV [1.;2.], ConstS 1.)
    | _ -> false
let%test "type_test52" =
  try
    let _ = type_of (Angle (T, ConstV [1.;2.])) in false
  with
...
let%test "type_test64" = (type_of (Cond (T, ConstV [1.;2.], ConstV [1.;2.])) = Vector 2)
let%test "type_test65" = (type_of (Cond (T, ConstV [1.;2.;3.;4.], ConstV [1.;2.;3.;4.])) = Vector 4)
let%test "type_test66" = 
  try
    let _ = type_of (Cond (T, ConstV [1.;2.], ConstS 1.)) in false
  with
    | Wrong e -> e = Cond (T, ConstV [1.;2.], ConstS 1.)
    | _ -> false
let%test "type_test67" =
  try
    let _ = type_of (Cond (T, ConstS 1., ConstV [1.;2.])) in false
  with
    | Wrong e -> e = Cond (T, ConstS 1., ConstV [1.;2.])
    | _ -> false
let%test "type_test68" =
  try
    let _ = type_of (Cond (T, ConstV [1.;2.], ConstV [1.;2.;3.])) in false
  with
    | Wrong e -> e = Cond (T, ConstV [1.;2.], ConstV [1.;2.;3.])
    | _ -> false
let%test "type_test69" = 
  try
    let _ = type_of (Cond (ConstS 1., T, T)) in false 
  with
    | Wrong e -> e = Cond (ConstS 1., T, T)
    | _ -> false

let%test "type_test70" = (type_of (UnitVector (ConstV [1.;2.])) = Vector 2)
let%test "type_test71" = (type_of (UnitVector (ConstV [1.;2.;3.;4.])) = Vector 4)
let%test "type_test72" = (type_of (UnitVector (ConstV [1.;2.;3.;4.;5.])) = Vector 5)
let%test "type_test73" = 
  try
    let _ = type_of (UnitVector (ConstS 1.)) in false
  with
    | Wrong e -> e = UnitVector (ConstS 1.)
    | _ -> false
let%test "type_test74" =
  try
    let _ = type_of (UnitVector (T)) in false
  with
    | Wrong e -> e = UnitVector (T)
    | _ -> false
let%test "type_test75" =
  try
    let _ = type_of (UnitVector (F)) in false
  with
    | Wrong e -> e = UnitVector (F)
    | _ -> false
let%test "type_test76" = (type_of (ZeroVector (ConstV [1.;2.])) = Vector 2)
let%test "type_test77" = (type_of (ZeroVector (ConstV [1.;2.;3.;4.])) = Vector 4)
let%test "type_test78" = (type_of (ZeroVector (ConstV [1.;2.;3.;4.;5.])) = Vector 5)
let%test "type_test79" = 
  try
    let _ = type_of (ZeroVector (ConstS 1.)) in false
  with
    | Wrong e -> e = ZeroVector (ConstS 1.)
    | _ -> false
let%test "type_test80" =
  try
    let _ = type_of (ZeroVector (T)) in false
  with
    | Wrong e -> e = ZeroVector (T)
    | _ -> false
let%test "type_test81" =
  try
    let _ = type_of (ZeroVector (F)) in false
  with
    | Wrong e -> e = ZeroVector (F)
    | _ -> false
let%test "type_test82" = (type_of (Sub (ConstS 1., ConstS 2.)) = Scalar)
let%test "type_test83" = (type_of (Sub (ConstV [1.;2.], ConstV [3.;4.])) = Vector 2)
let%test "type_test84" = (type_of (Sub (ConstV [1.;2.;3.;4.], ConstV [3.;4.;5.;6.])) = Vector 4)
let%test "type_test85" = 
  try
    let _ = type_of (Sub (ConstV [1.;2.], ConstV [3.;4.;5.])) in false
  with
    | Wrong e -> e = Sub (ConstV [1.;2.], ConstV [3.;4.;5.])
    | _ -> false
let%test "type_test86" =
  try
    let _ = type_of (Sub (ConstV [1.;2.], ConstS 1.)) in false
  with
    | Wrong e -> e = Sub (ConstV [1.;2.], ConstS 1.)
    | _ -> false
let%test "type_test87" =
  try
    let _ = type_of (Sub (T, ConstV [1.;2.])) in false
  with
...
let%test "eval_test2" = (eval (Add (ConstV [1.;2.], ConstV [3.;4.])) = V [4.;6.])
let%test "eval_test3" = (eval T = B true)
let%test "eval_test4" = (eval F = B false)
let%test "eval_test5" = (eval (ConstS 1.) = S 1.)
let%test "eval_test6" = (eval (ConstV [1.;2.; 3.; 4.; 5.]) = V [1.;2.; 3.; 4.; 5.])
...
let%test "eval_test11" = 
  try
    let _ = eval (Add (T, ConstS 1.)) in false
  with
    | Wrong e -> e = Add (T, ConstS 1.)
    | _ -> false
let%test "eval_test12" =
  try
    let _ = eval (Add (ConstS 2., ConstV [1.;2.])) in false
  with
    | Wrong e -> e = Add (ConstS 2., ConstV [1.;2.])
    | _ -> false  
let%test "eval_test13" =
  try
    let _ = eval (Add (ConstV [1.;2.], T)) in false
  with
    | Wrong e -> e = Add (ConstV [1.;2.], T)
    | _ -> false
...
let%test "eval_test30" = (eval (ScalProd ((Inv (ConstV [1.;2.])), ConstS 2.)) = V [-2.;-4.])
let%test "eval_test31" = (eval (ScalProd (ConstV [1.;2.], ConstS 1.)) = V [1.;2.])
let%test "eval_test32" = 
  try
    let _ = eval (ScalProd (T, ConstS 1.)) in false
  with
    | Wrong e -> e = ScalProd (T, ConstS 1.)
    | _ -> false
let%test "eval_test33" =
  try
    let _ = eval (ScalProd (T, ConstV [1.;2.])) in false
  with
    | Wrong e -> e = ScalProd (T, ConstV [1.;2.])
    | _ -> false
let%test "eval_test34" =
  try
    let _ = eval (ScalProd (ConstV [1.; 3.], ConstV [1.;2.])) in false
  with
    | Wrong e -> e = ScalProd (ConstV[1.; 3.], ConstV [1.;2.])
    | _ -> false
let%test "eval_test35" = (eval (DotProd (ConstV [1.;2.], ConstV [3.;4.])) = S 11.)
let%test "eval_test36" = (eval (DotProd (ConstV [1.;2.;3.;4.], ConstV [3.;4.;5.;6.])) = S 50.)
let%test "eval_test37" = (eval (DotProd (ConstV [1.;2.;3.;4.;5.], ConstV [3.;4.;5.;6.;7.])) = S 85.)
let%test "eval_test38" = 
  try
    let _ = eval (DotProd (ConstV [1.;2.], ConstV [3.;4.;5.])) in false
  with
    | Wrong e -> e = DotProd (ConstV [1.;2.], ConstV [3.;4.;5.])
    | _ -> false
let%test "eval_test39" =
  try
    let _ = eval (DotProd (ConstV [1.;2.], ConstS 1.)) in false
  with
    | Wrong e -> e = DotProd (ConstV [1.;2.], ConstS 1.)
    | _ -> false
let%test "eval_test40" =
  try
    let _ = eval (DotProd (T, ConstV [1.;2.])) in false
  with
    | Wrong e -> e = DotProd (T, ConstV [1.;2.])
    | _ -> false
...
let%test "eval_test46" =
  try
    let _ = eval (Mag (F)) in false
  with
    | Wrong e -> e = Mag (F)
    | _ -> false
let%test "eval_test47" = (eval (Angle (ConstV [1.;2.], ConstV [3.;4.])) = S (acos (11./.(sqrt 25. *. sqrt 5.))))
let%test "eval_test48" = (eval (Angle (ConstV [1.;2.;3.;4.], ConstV [3.;4.;5.;6.])) = S (acos (50./.(sqrt 30. *. sqrt 86.))))
let%test "eval_test49" = (eval (Angle (ConstV [1.;2.;3.;4.;5.], ConstV [3.;4.;5.;6.;7.])) = S (acos (85./.(sqrt 55. *. sqrt 135.))))
let%test "eval_test50" = 
  try
    let _ = eval (Angle (ConstV [1.;2.], ConstV [3.;4.;5.])) in false
  with
    | Wrong e -> e = Angle (ConstV [1.;2.], ConstV [3.;4.;5.])
    | _ -> false
let%test "eval_test51" =
  try
    let _ = eval (Angle (ConstV [1.;2.], ConstS 1.)) in false
  with
    | Wrong e -> e = Angle (ConstV [1.;2.], ConstS 1.)
    | _ -> false
let%test "eval_test52" =
  try
    let _ = eval (Angle (T, ConstV [1.;2.])) in false
  with
    | Wrong e -> e = Angle (T, ConstV [1.;2.])
    | _ ->
      false
...
let%test "eval_test64" = (eval (Cond (T, ConstV [1.;2.], ConstV [1.;2.])) = V [1.;2.])
let%test "eval_test65" = (eval (Cond (T, ConstV [1.;2.;3.;4.], ConstV [1.;2.;3.;4.])) = V [1.;2.;3.;4.])
let%test "eval_test66" = 
  try
    let _ = eval (Cond (T, ConstV [1.;2.], ConstS 1.)) in false
  with
    | Wrong e -> e = Cond (T, ConstV [1.;2.], ConstS 1.)
    | _ -> false
let%test "eval_test67" =
  try
    let _ = eval (Cond (T, ConstS 1., ConstV [1.;2.])) in false
  with
    | Wrong e -> e = Cond (T, ConstS 1., ConstV [1.;2.])
    | _ ->
      false
let%test "eval_test68" =
  try
    let _ = eval (Cond (T, ConstV [1.;2.], ConstV [1.;2.;3.])) in false
  with
    | Wrong e -> e = Cond (T, ConstV [1.;2.], ConstV [1.;2.;3.])
    | _ -> false
let%test "eval_test69" =
  try
    let _ = eval (Cond (ConstS 1., T, T)) in false
  with
    | Wrong e -> e = Cond (ConstS 1., T, T)
    | _ ->
      false
let%test "eval_test70" = (eval (UnitVector (ConstV [1.;2.])) = V [1./.(sqrt 5.); 2./.(sqrt 5.)])
let%test "eval_test71" = (eval (UnitVector (ConstV [1.;2.;3.;4.])) = V [1./.(sqrt 30.); 2./.(sqrt 30.); 3./.(sqrt 30.); 4./.(sqrt 30.)])
let%test "eval_test72" = (eval (UnitVector (ConstV [1.;2.;3.;4.; 5.])) = V [1. *. (1. /. (sqrt 55.)); 2. *. (1. /. (sqrt 55.)); 3. *. (1. /. (sqrt 55.)); 4. *. (1. /. (sqrt 55.)); 5. *. (1. /. (sqrt 55.))])
let%test "eval_test73" = 
  try
    let _ = eval (UnitVector (ConstS 1.)) in false
  with
    | Wrong e -> e = UnitVector (ConstS 1.)
    | _ ->
      false
let%test "eval_test74" =
  try
    let _ = eval (UnitVector (T)) in false
  with
    | Wrong e -> e = UnitVector (T)
    | _ ->
      false
let%test "eval_test75" =
  try
    let _ = eval (UnitVector (F)) in false
  with
    | Wrong e -> e = UnitVector (F)
    | _ ->
      false
let%test "eval_test76" = (eval (ZeroVector (ConstV [1.;2.])) = V [0.;0.])
let%test "eval_test77" = (eval (ZeroVector (ConstV [1.;2.;3.;4.])) = V [0.;0.;0.;0.])
let%test "eval_test78" = (eval (ZeroVector (ConstV [1.;2.;3.;4.;5.])) = V [0.;0.;0.;0.;0.])
let%test "eval_test79" = 
  try
    let _ = eval (ZeroVector (ConstS 1.)) in false
  with
    | Wrong e -> e = ZeroVector (ConstS 1.)
    | _ ->
      false
let%test "eval_test80" =
  try
    let _ = eval (ZeroVector (T)) in false
  with
    | Wrong e -> e = ZeroVector (T)
    | _ ->
      false
let%test "eval_test81" =
  try
    let _ = eval (ZeroVector (F)) in false
  with
    | Wrong e -> e = ZeroVector (F)
    | _ ->
      false
let%test "eval_test82" = (eval (Sub (ConstS 1., ConstS 2.)) = S (-1.))
let%test "eval_test83" = (eval (Sub (ConstV [1.;2.], ConstV [3.;4.])) = V [-2.; -2.])
let%test "eval_test84" = (eval (Sub (ConstV [1.;2.;3.;4.], ConstV [3.;4.;5.;6.])) = V [-2.; -2.; -2.; -2.])
let%test "eval_test85" = 
  try
    let _ = eval (Sub (ConstV [1.;2.], ConstV [3.;4.;5.])) in false
  with
    | Wrong e -> e = Sub (ConstV [1.;2.], ConstV [3.;4.;5.])
    | _ ->
      false
let%test "eval_test86" =
  try
    let _ = eval (Sub (ConstV [1.;2.], ConstS 1.)) in false
  with
    | Wrong e -> e = Sub (ConstV [1.;2.], ConstS 1.)
    | _ ->
      false
let%test "eval_test87" =
  try
    let _ = eval (Sub (T, ConstV [1.;2.])) in false
  with
    | Wrong e -> e = Sub (T, ConstV [1.;2.])
    | _ ->
      false
