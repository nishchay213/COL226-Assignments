...
let () =
  ...
  assert_test (Vector.dim v1 = 3);
  assert_test (Vector.dim v2 = 5);
  assert_test (Vector.dim v3 = 1);
  Printf.printf "dim passed!\n";

  Printf.printf "Testing is_zero...\n";
  assert_test (Vector.is_zero [0.; 0.; 0.]);
  assert_test (not (Vector.is_zero [0.; 1.; 0.]));
  assert_test (not (Vector.is_zero [1.0]));
  Printf.printf "is_zero passed!\n";

  Printf.printf "Testing unit...\n";
  assert_test (Vector.unit 3 2 = [0.; 1.; 0.]);
  assert_test (Vector.unit 4 1 = [1.; 0.; 0.; 0.]);
  assert_test (Vector.unit 5 5 = [0.; 0.; 0.; 0.; 1.]);
  ...
  assert_test (Vector.addv [1.; 2.; 3.] [4.; 5.; 6.] = [5.; 7.; 9.]);
  assert_test (Vector.addv [0.; 0.] [0.; 0.] = [0.; 0.]);
  ...
  assert_test (Vector.dot_prod [1.; 2.; 3.] [4.; 5.; 6.] = 32.);
  assert_test (Vector.dot_prod [1.; 0.; -1.] [0.; 1.; 0.] = 0.);
  ...
  assert_test (Vector.inv [1.; -2.; 3.] = [-1.; 2.; -3.]);
  assert_test (Vector.inv [0.; 0.; 0.] = [0.; 0.; 0.]);
  Printf.printf "inv passed!\n";

  Printf.printf "Testing length...\n";
  assert_test (Vector.length [3.; 4.] = 5.);
  assert_test (Vector.length [0.; 0.; 0.] = 0.);
  Printf.printf "length passed!\n";

  Printf.printf "Testing angle...\n";
  assert_test (abs_float (Vector.angle [1.; 0.] [0.; 1.] -. (Float.pi /. 2.)) < 1e-6);
  assert_test (abs_float (Vector.angle [1.; 1.] [1.; 1.] -. 0.) < 1e-6);
  assert_test (abs_float (Vector.angle [1.; 0.; 0.] [0.; 1.; 0.] -. (Float.pi /. 2.)) < 1e-6);
  assert_test (abs_float (Vector.angle [1.; 2.; 3.] [-1.; -2.; -3.] -. Float.pi) < 1e-6);
  assert_test (abs_float (Vector.angle [1.; 2.; 2.] [2.; 1.; 2.] -. 0.4758822496) < 1e-6);
